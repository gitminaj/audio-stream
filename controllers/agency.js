...
export const register = async (req, res) =>{
    ...
    try {
        const { name, agencyName, email, number, gender, idProofName, accountNumber, IFSC } = req.body;

        if( !name || !agencyName || !email || !number || !gender || !idProofName || !accountNumber || !IFSC){
        ...
        const agencyIdProofFile = req.files?.agencyIdProofFile[0]?.location || null;
